  ...
  let filteredPairs = $derived(
    parentPairs.filter((pair) => {
      if (reverseParentFilter) {
        if (
          pair.parent1.id !== reverseParentFilter.id &&
          pair.parent2.id !== reverseParentFilter.id
        )
          return false;
      }
...
<div class="w-3xl space-y-4">
  <div class="flex gap-4">
    ...
    <div>
      <span class="text-muted-foreground mb-1 block text-sm">Filter parent</span>
      <PalAutocomplete
  ...
  <div class="overflow-x-auto rounded-md border border-neutral-800">
    <Table.Root class="text-sm">
      <Table.Header>
        <Table.Row>
          <th
            class="text-muted-foreground w-1/2 cursor-pointer px-3 text-left text-sm font-medium select-none"
