<script lang="ts">
	...
	type AllSortKey = 'parent' | 'child' | 'work' | 'mount' | 'rarity' | 'slow' | 'walk' | 'run' | 'sprint' | 'tpot' | 'swim' | 'dash' | 'stam' | 'hp' | 'atk' | 'def' | 'food' | 'price';
	...
	function getSortValue(row: BreedingAllResult, key: AllSortKey): number {
		switch (key) {
			case 'parent': return parseInt(row.parent.number) || 0;
			case 'child': return parseInt(row.child.number) || 0;
			case 'work': return row.child.workSuitabilities.reduce((sum, w) => sum + w.level, 0);
			case 'mount': return row.child.mounts.length > 0 ? row.child.mounts[0].unlockLevel : 9999;
			case 'rarity': return row.child.stats?.rarity ?? 0;
			case 'slow': return row.child.movement?.slowWalkSpeed ?? 0;
			case 'walk': return row.child.movement?.walkSpeed ?? 0;
			case 'run': return row.child.movement?.runSpeed ?? 0;
			case 'sprint': return row.child.movement?.rideSprintSpeed ?? 0;
			case 'tpot': return row.child.movement?.transportSpeed ?? 0;
			case 'swim': return row.child.movement?.swimSpeed ?? 0;
			case 'dash': return row.child.movement?.swimDashSpeed ?? 0;
			case 'stam': return row.child.movement?.stamina ?? 0;
			case 'hp': return row.child.stats?.health ?? 0;
			case 'atk': return row.child.stats?.attack ?? 0;
			case 'def': return row.child.stats?.defense ?? 0;
			case 'food': return row.child.stats?.food ?? 0;
			case 'price': return row.child.stats?.price ?? 0;
		}
	}
...
<div class="space-y-4">
	<div class="flex gap-4">
		<div>
			<span class="mb-1 block text-sm text-muted-foreground">Parent</span>
			<PalAutocomplete
				pals={pals}
		...
		<div>
			<span class="mb-1 block text-sm text-muted-foreground">Filter child</span>
			<PalAutocomplete
	...
	{:else if allResults.length > 0}
		<div class="text-sm text-muted-foreground">
			{new Set(sortedResults.map(r => r.child.id)).size}/{new Set(allResults.map(r => r.child.id)).size} unique children
		</div>
	{/if}
			...
			<Table.Header>
				<Table.Row>
					{#each [
						...
						{ key: 'def', label: 'DEF' },
						{ key: 'food', label: 'Food' },
						{ key: 'price', label: 'Coin' }
					] as col (col.key)}
						<th
							class="cursor-pointer select-none px-2 text-center text-sm font-medium text-muted-foreground"
