<script lang="ts">
	...
	function restoreState(pals: PalRow[]): void {
		try {
			...
			if (state.singleParentId) singleParent = pals.find((p) => p.id === state.singleParentId) ?? null;
			if (state.reverseChildId) reverseChild = pals.find((p) => p.id === state.reverseChildId) ?? null;
...
<div class="mx-auto flex h-full max-w-[90rem] flex-col px-6 pt-8">
	<Tabs.Root bind:value={activeTab}>
		<div class="flex justify-center">
			<Tabs.List>
				<Tabs.Trigger value="parents-child" class="data-[state=active]:bg-white/10 data-[state=active]:text-white">Parents &rarr; Child</Tabs.Trigger>
				<Tabs.Trigger value="one-parent" class="data-[state=active]:bg-white/10 data-[state=active]:text-white">One Parent &rarr; Childs</Tabs.Trigger>
				<Tabs.Trigger value="child-parents" class="data-[state=active]:bg-white/10 data-[state=active]:text-white">Child &rarr; Parents</Tabs.Trigger>
			</Tabs.List>
		</div>
