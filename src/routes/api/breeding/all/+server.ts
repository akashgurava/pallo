...
export const GET: RequestHandler = async ({ url }) => {
  ...
  try {
    ...
    const combos = await db
      ...
      .from(breedingCombos)
      .where(or(eq(breedingCombos.parent1Id, parentId), eq(breedingCombos.parent2Id, parentId)))
      .all();
    ...
    for (const combo of combos) {
      ...
      const [otherParent, childPal] = await Promise.all([
        db
          ...
          .from(pals)
          .where(eq(pals.id, otherParentId))
          .get(),
        db
          .select({ id: pals.id, number: pals.number, variant: pals.variant, name: pals.name })
          .from(pals)
      ...
      const [
        ...
        db
          ...
          .where(eq(palMounts.palId, otherParent.id))
          .all(),
        db
          .select({ name: elements.name })
          .from(palElements)
        ...
        db
          ...
          .from(palMounts)
          .innerJoin(mountTypes, eq(palMounts.mountTypeId, mountTypes.id))
          .where(eq(palMounts.palId, childPal.id))
          .all(),
        db
          .select({
            size: palStats.size,
          ...
          .from(palStats)
          .where(eq(palStats.palId, childPal.id))
          .get(),
        db
          .select({
          ...
          .from(palMovement)
          .where(eq(palMovement.palId, childPal.id))
          .get(),
      ]);
