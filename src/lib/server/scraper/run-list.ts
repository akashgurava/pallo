...
async function seedOrderedLookups(elementOrder: string[], workTypeOrder: string[]): Promise<void> {
...
async function insertPal(
  ...
  for (const elementName of entry.elements) {
    const elementId = await getOrCreateElement(elementName, elementOrderMap.get(elementName) ?? 99);
